...
- [奕码](https://yima.pages.dev/grand-code/)：三码定长或者四二顶〔26*26=676个二码字不需空格、会自动顶；三码需空格〕，小码是字根的拼音，重码少。26 键，出简不出全。纯**单字**码表，覆盖 7900 个字。编码最短 2 码，最长 3 码。（类似：虎码 + 顶功。 为了用更短码长，打小字集的字，可是字根也不少，有一定的记忆量。）

- [易码](https://yima.pages.dev/easy-code/)：三码定长或者四二顶〔26*26=676个二码字不需空格、会自动顶；三码需空格〕，总字根数 440+，归类完 268+，采用双编〔大码乱序+小码有序〕分布，小码二笔。(类似：五笔 + 二笔 + 顶功)
