...
大陆的《新华字典》，每个字除了标了拼音，也标着注音，几十年了多个版本修订，到现在的最新的 第 12 版，依旧保留有注音。注音符号本来就是源自汉字的偏旁部首。大陆人稍加学习，也能掌握注音。
...
从一推出时，就面向大字符集（数以万计的字的集合），不像五笔那样最初只面对GB2312的六千多字来设计字根，后来还推出98版、新世纪版等多个版本。

若以大陆字型为准，编码与五笔相比，更规范。拆字更舒服。

不满意郑码者，是郑码规则导致的“简全不一致”（简码和词组编码跟全码不同，简码并不是全码前几键）

受郑码字根双编码的思路影响，近些年出现了一些字根很多的字根双编码输入方案。
...
另外，有些字拆字别扭。（当然，汉字几千年的演变，非常复杂，没有一种形码，拆字会绝对舒服，绝对规范的。况且也没绝对的《规范》可供所有汉字使用者遵守）
