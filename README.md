...
大部分人，三四千汉字已满足现代汉语日常交流的 99% 以上的使用。（日本现在大约使用两千日文汉字。）
...
大陆的《新华字典》，每个字除了标了拼音，也标着注音，几十年了多个版本修订，到现在的最新的 第 12 版，依旧保留有注音。大陆人只要稍加学习下，也能掌握注音。注音符号本来都是源自汉字的偏旁部首。
...
不满意者，通常是郑码的“简全不一致”（简码和词组编码跟全码不同，简码并不是全码前几键）

受其字根双编码的思路影响，近些年出现了一些字根很多的字根双编码输入方案。
...
五笔、郑码等，需要看书或通过培训班等，学习掌握复杂的规则、特例，甚至还要记无理码。 
...
这是虎码网站上提供的一个测评工具。任何输入方案的码表（必要时转换一下格式），上传后，可得到常用6000字的该输入方案的测评情况：
