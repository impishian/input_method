...
################
# 鹤形辅码
awk '{print $2}' xh_8105.txt | sed -r 's/\[//g' | awk '{count[$0]++; print count[$0] "=" $0}' > 3_xh.txt
#
# 码数: 7881
awk '{print $2}' xh_8105.txt | awk '!a[$0]++' | wc -l
#
# 自然鹤形
paste -d "," 0.txt 3_xh.txt | sd "," ",1="  > 4_xh.txt

################
# 自然辅码
awk '{print $2}' zrm_8105.txt | sed -r 's/\[//g' | awk '{count[$0]++; print count[$0] "=" $0}' > 3_zrm.txt
#
# 码数: 7012
awk '{print $2}' zrm_8105.txt | awk '!a[$0]++' | wc -l
#
# 自然双拼+自然形
paste -d "," 0.txt 3_zrm.txt  > 4_zrm.txt

################
# 虎码首末辅码
awk '{print $2}' new_8105.txt| awk -F ';'  '{print $7}' > 2_huma.txt
paste -d " " 1.txt 2_huma.txt | sed -r 's/ //g' | awk '{count[$0]++; print count[$0] "=" $0}' > 3_huma.txt
#
# 码数: 7996
paste -d " " 1.txt 2_huma.txt | awk '!a[$0]++' | wc -l
#
# 自然虎码
paste -d "," 0.txt 3_huma.txt > 4_huma.txt

################
# 五笔前2辅码
awk '{print $2}' new_8105.txt| awk -F ';'  '{print $8}' > 2_wubi.txt
paste -d " " 1.txt 2_wubi.txt | sed -r 's/ //g' | awk '{count[$0]++; print count[$0] "=" $0}' > 3_wubi.txt
#
# 码数: 7526 
...
#
# 自然五笔
# paste -d "," 0.txt 3_wubi.txt |  > 4_wubi.txt
